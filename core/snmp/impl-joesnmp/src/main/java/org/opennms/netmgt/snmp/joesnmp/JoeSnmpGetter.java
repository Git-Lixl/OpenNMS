...
public class JoeSnmpGetter extends SnmpGetter {
    
    static public abstract class JoeSnmpPduBuilder extends PduBuilder {
    ...
    public static class GetBuilder extends JoeSnmpPduBuilder {
        private SnmpPduRequest m_getPdu = null;
        ...
        public void reset() {
            m_getPdu = new SnmpPduRequest(SnmpPduRequest.GET);
            m_getPdu.setRequestId(SnmpPduPacket.nextSequence());
        }
    ...
    public JoeSnmpGetter(JoeSnmpAgentConfig agentConfig, String name, CollectionTracker tracker) {
        super(agentConfig.getAddress(), name, agentConfig.getMaxRepetitions(), agentConfig.getMaxVarsPerPdu(),tracker);
    ...
    public void start() {
        log().info("get "+getName()+" for "+getAddress()+" using version "+SnmpSMI.getVersionString(getVersion())+" with config: "+m_agentConfig);
        super.start();
    }

	@Override
    protected PduBuilder createPduBuilder(int maxVarsPerPdu) {
        return  (JoeSnmpPduBuilder)new GetBuilder(maxVarsPerPdu);
    }

    protected void sendPdu(PduBuilder pduBuilder) throws SocketException {
