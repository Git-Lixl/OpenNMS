 ...
 */
@Distributable
public class RancidHealthMonitor extends IPv4Monitor {
                ...
                try {
                    ...
                    if (httpClient.getPollStatus() == PollStatus.SERVICE_AVAILABLE && StringUtils.isNotBlank(httpClient.getResponseText())) {
                        ...
                        if (!httpClient.isResponseTextFound()) {
                            String message = httpClient.getReasonFromHtmlBody();
                            log().debug(message);
                            httpClient.setReason(message);
                        }
                    }
    ...
    final class HttpMonitorClient {
        ...
        public String getReasonFromHtmlBody() {
        	int start = m_html.indexOf("<HTML><BODY>");
        	int end = m_html.indexOf("</HTML></BODY>");
        	return m_html.substring(start+12, end);
        }
