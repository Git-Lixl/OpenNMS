...
public class MinionProvisionTest {
    ...
    public void checkServicesDetectedOnMinion() throws ClientProtocolException, IOException, InterruptedException {
        ...
        await().atMost(3, MINUTES).pollInterval(30, SECONDS).until(getnumberOfServicesDetected(client), greaterThan(0));
        ...
        return new Callable<Integer>() {
            public Integer call() throws Exception {
                ...
                if (!CollectionUtils.isEmpty(nodes)) {
                    OnmsNode retrievedNode = nodes.get(0);
                    System.out.printf("Retrieved node Id = %s   label = %s ", retrievedNode.getNodeId(),
                            retrievedNode.getLabel());
                    List<OnmsMonitoredService> services = client.getServicesForANode(retrievedNode.getNodeId(),
                            LOCALHOST);
